...
const main = async () => {
  const argv = await yargs(process.argv.slice(2))
    .option("mnemonic", {
      ...
      coerce: (arg: string) => arg?.trim()  // Added optional chaining
    })
    .option("p", {
  ...
  if (argv.test) {
    ...
    if (!argv.mnemonic) throw new Error("Mnemonic is required");
    await generateKeys(argv.mnemonic, argv.p, argv.n);
  }
};


async function generateKeys(mnemonic: string, includePk: boolean, count: number) {
